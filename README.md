...
3.  Follow the on-screen installation steps. You will need to:
    *   Provide your SMTP server's details (e.e., from SendGrid, Mailgun, or another provider).
...
The `tests` directory is organized to separate different types of tests:

- `tests/unit_test/`: Contains general unit tests for individual functions or components.
- `tests/data_tests/`: Contains unit tests specifically for the data repositories (e.g., Firebase data access). These tests verify the data layer logic in isolation.
- `tests/e2e_test/`: Contains end-to-end tests using Playwright to simulate user workflows in a browser environment.
- `tests/reports/`: This folder is intended to store test reports generated by the test runners (Vitest and Playwright).
